...
def generate_docx_from_template(
    data: Union[str, Iterable[Union[str, Tuple[str, str]]]],
    template_path: str,
    output_path: str,
) -> None:
    """
    ...
    for item in data_iter:
        if isinstance(item, str):
            ...
            if not item:
                continue
            if ":" not in item:
                continue  # при желании можно логировать
            key, value = item.split(":", 1)
        else:
            # предполагаем кортеж/список (key, value)
            if len(item) != 2:
                continue
            key, value = item
        ...
        context[key] = value
        print(key, value)
    print("Материалы для слияния ", context)
